...
export default class Wujie {
  ...
  public async active(options: {
    ...
    const { sync, url, el, props, alive, prefix, fetch, replace } = options;
    this.url = url;
    this.sync = sync;
    this.alive = alive;
    this.hrefFlag = false;
    ...
    if (this.shadowRoot) {
       ...
       */
      this.el = renderElementToContainer(this.shadowRoot, el);
  ...
  public patchCssRules(): void {
    ...
    if (this.shadowRoot.hasAttribute(WUJIE_DATA_ATTACH_CSS_FLAG)) return;
    ...
    if (fontStyleSheetElement) {
      this.shadowRoot.appendChild(fontStyleSheetElement);
    }
    (hostStyleSheetElement || fontStyleSheetElement) &&
      this.shadowRoot.setAttribute(WUJIE_DATA_ATTACH_CSS_FLAG, "");
  }
